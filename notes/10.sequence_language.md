...
$$P(w_T) = \frac{\text{count}(w_T)}{\text{total\_words}}$$
...
At each time step $t$, the hidden state $\mathbf{h}_t$ is updated using $ \mathbf{h}_t = \phi(\mathbf{W_h} \mathbf{h}_{t-1} + \mathbf{W_x} \mathbf{x}_t + \mathbf{b}) $.  
